...
- 横断面图配置命令：命令 `RD_SECTION_DRAWING_CONFIG`
  ...
  - 窗口上方支持 CSV 配置路径、导入和导出；当前包含 `路面结构层` tab 和 `清表` tab。路面结构层表格字段为起点桩号、终点桩号、路基类型多选和模板；清表表格字段为起点桩号、终点桩号、左侧坡率、右侧坡率、厚度、作用范围和挖方是否清表。
...
- 出图出表规则：路面工程量统计表命令元数据、横断面图实体当前面域优先采样、构造物切段、旧道路模型部件名反推、部件/结构层双模式聚合、结构层平面投影面积、平均断面法体积和带格式 `.xls` 写出；路面结构图例命令元数据、模板列规划、表头列宽、结构图示不写层类型文字、厚度厘米表达、底部图例不合并和普通 CAD 图元绘制契约。
