...
- 统一传递模板显示方式和每层填充类型；C++ 侧继续通过领域规则归一化非法显示方式或填充名。
...
- 模板字段：模板名称、显示比例、预览宽度、显示方式、插入点。
- 层级字段：层数、层类型、层名、RGB 颜色、填充类型、等厚标志、单一厚度、内侧厚度、外侧厚度、内侧加宽、外侧加宽、内侧坡度、外侧坡度。
...
- 显示方式写出为 `displayMode`，每层填充写出为 `layer.N.hatchPattern`；WPF 预览和 `DnPavementLayerTemplateEntity` 按该模式显示，`DnRoadModelEntity` 仍保持颜色模式。
