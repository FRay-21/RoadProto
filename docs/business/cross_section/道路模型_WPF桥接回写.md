...
- 用请求文件把道路模型配置传给 WPF。
- 用响应文件从 WPF 回传采样间距、路基模板范围表、左右边坡模板组、搜索宽度、构造物范围表、目标道路模型 handle 和点选模板动作。
...
7. 生成模型时，命令解析道路中线、竖曲线、路基模板、路面结构层模板、边坡模板、构造物范围、道路模型 handle 和可用 TIN 地面数据。
...
- `structureCount` 受桥接层上限保护，构造物行通过 `structure.{i}.startStation`、`structure.{i}.endStation`、`structure.{i}.type`、`structure.{i}.sideRange` 回传。
- 构造物类型稳定编码为 `Bridge`、`Tunnel`；影响范围稳定编码为 `Left`、`Right`、`Both`，WPF 显示文本为桥梁、隧道、左侧、右侧、两侧。
- 采样间距、桩号范围、搜索宽度和模板 handle 由 C++ application/domain 层再次校验。
- 构造物起终点和影响范围由 C++ Bridge 和 domain 层校验；无效范围会拒绝构建，不创建半成品模型。
...
- 可复用内容：道路模型 DTO、构造物范围 DTO、请求/响应文件桥接、C++ 回写命令结构
