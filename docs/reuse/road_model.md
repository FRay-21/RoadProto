...
道路模型能力把道路中线、纵断面竖曲线设计高程、横断面路基模板、路面结构层模板和边坡模板组合为三维道路模型线框。它可复用于后续三维建模、出图、算量、检查和道路对象联动。
...
- 竖曲线高程与道路中线平面点组合。
- 路基模板部件边界转三维部件线。
- 读取路基模板部件绑定的路面结构层模板，并生成 `RoadModelWireLineKind::PavementLayer` 三维结构层线框。
...
- 当前只生成三维线框，不生成道路实体面、材质、结构层实体体积或算量结果；路基/道路主体颜色沿用部件颜色，边坡线颜色来自边坡模板部件颜色，结构层线按结构层类型表达。
...
- AutoCAD 手工验证：创建道路中线、竖曲线、路基模板和路面结构层模板后运行 `RD_SECTION_ROAD_MODEL_CREATE`，在路基模板部件中点选绑定结构层模板，在道路模型表格行内点选图中模板、在边坡模板组内管理多个模板，并确认生成时状态栏显示进度且最终创建 `DnRoadModelEntity`；双击道路模型或运行 `RD_SECTION_ROAD_MODEL_EDIT` 后可调整模板范围并刷新实体；运行 `RD_SECTION_ROAD_MODEL_VIEW_SECTION` 后选择道路模型，应能按采样桩号切换并预览路基模板线、结构层线、边坡模板线和生成时地面线快照。
