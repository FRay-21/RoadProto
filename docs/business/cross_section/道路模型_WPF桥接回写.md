...
- 创建或更新道路模型时一并保存生成采样桩号、断面节点链、地面剖面快照和三维网格线框，供 CAD 显示和 `查看横断面` 读取。
...
- 领域实体：`RoadModelData`，包含三维部件线、断面节点链、网格线框和采样桩号
- 表格或报告：无
...
7. 生成模型时，命令解析道路中线、竖曲线、路基模板、边坡模板、道路模型 handle 和可用 TIN 地面数据。
