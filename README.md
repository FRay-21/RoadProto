...
- 版本：`v0.1.27`
- 构建日期：`20260527`
- 阶段：`RoadModelStructures`
- ARX 文件名：`RoadProto_v0.1.27_20260527_RoadModelStructures.arx`
...
```text
ARXLOAD artifacts\x64\Release\RoadProto_v0.1.27_20260527_RoadModelStructures.arx
...
- 横断面道路模型规则：模板优先级解析、采样点收集、竖曲线高程、路基模板三维部件线生成、路面结构层模板内外侧语义和线框生成、边坡模板默认值和几何约束、边坡模板组优先级、构造物范围按左侧/右侧/两侧跳过边坡放坡、TIN 地面剖切交地、断面地面快照、断面节点链、三维网格线框生成、生成进度回调、采样桩号持久化、横断面查看预览、命令元数据、WPF Bridge 和 ObjectARX 实体源码契约。
