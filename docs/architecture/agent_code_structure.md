...
| `Services/AgentBackendProcess.cs` | 在 WPF 面板打开时启动本地 sidecar，并在面板关闭或托管插件卸载时关闭本次由面板启动的进程 |
...
WPF 面板可以管理本地 sidecar 的进程生命周期，但只能关闭自己本次自动启动的 `RoadProto.Agent.Host` 进程；如果 `/health` 已经可用，视为外部后端并只复用连接，不在面板关闭时强制结束。
