...
托管 WPF 构建覆盖右侧 Agent 面板、`RD_AI_ASSISTANT_OPEN`、后端 HTTP 客户端、自动启动/关闭本地 sidecar、中文 IME 输入焦点保护、工具确认卡片、受控请求/结果文件路径生成和 Ribbon `Agent / AI 助手` 入口源码。

管理控制台测试覆盖配置读写、DPAPI 密钥保存、模型 Profile 管理 API、Markdown 上传与启用禁用、prompt 上下文组装、`/admin` 静态页面和 `/api/chat` 使用运行期默认模型配置。

AutoCAD 图形界面仍需手工验证：加载 ARX 和 `RoadProto.Terrain.UI.dll`，运行 `RD_AI_ASSISTANT_OPEN` 并确认 `RoadProto.Agent.Host` 自动启动，输入中文拼音并选择候选词确认文字停留在 Agent 输入框，打开 `http://127.0.0.1:17831/admin` 配置模型、API Key、skill 和知识库 Markdown，输入路基模板创建需求，确认工具卡片，检查 `%TEMP%\RoadProtoAgent\` 结果 JSON，并确认 DWG 中生成 `DnSubgradeTemplateEntity`；关闭 Agent 面板后确认由面板启动的后端退出。
