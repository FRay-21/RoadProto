...
- 版本：`v0.1.19`
- 构建日期：`20260520`
- 阶段：`PavementLayerTemplate`
- ARX 文件名：`RoadProto_v0.1.19_20260520_PavementLayerTemplate.arx`
...
- 路面结构层模板命令：命令 `RD_SECTION_PAVEMENT_LAYER_TEMPLATE_CREATE`
  ...
  - 用户点取插入点后生成独立 `DnPavementLayerTemplateEntity`，并打开 WPF 参数窗口编辑模板名称、显示比例、预览宽度和结构层列表。
  - 结构层类型下拉值固定为上面层、中面层、下面层、基层、底基层、垫层。
  - 厚度支持“内外厚度是否一致”复选框；一致时使用单一厚度，不一致时分别使用内侧厚度和外侧厚度。
  - 内侧表示更靠近道路中线的一侧，外侧表示更远离道路中线的一侧；内侧加宽向道路中线方向扩展。
...
- 横断面戴帽道路模型命令：命令 `RD_SECTION_ROAD_MODEL_CREATE`、`RD_SECTION_ROAD_MODEL_EDIT`
  ...
  - 生成时只取道路中线、竖曲线和模板范围的有效交集，读取路基模板部件绑定的路面结构层模板并生成结构层三维线框；边坡从路基模板最外侧开始，按左右搜索宽度从道路中线向外搜索 TIN 地面线；TIN 剖切使用网格候选过滤，并把生成时地面剖面快照保存到道路模型断面。
...
```text
ARXLOAD artifacts\x64\Release\RoadProto_v0.1.19_20260520_PavementLayerTemplate.arx
