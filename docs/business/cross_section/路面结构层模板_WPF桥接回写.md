...
- 统一传递 `showCreateWizard` 请求标记；当前新建流程收到该标记后直接套用“沥青路面-主线行车道”预设并打开参数窗口，保留原创建向导代码但不显示向导；按 handle 编辑既有实体时保持关闭。
...
- 结构层类型编码对应 WPF 下拉显示：上面层、中面层、下面层、沥青封层、基层、底基层、垫层、搭板。
...
- 每层材料名称仍按 `layer.N.name` 原样写出和读取；WPF 侧的材料下拉推荐项不进入 Bridge 协议，也不限制 C++ 回写。用户输入列表外材料时，桥接层必须当作普通字符串保留。
...
- 新建请求写出 `showCreateWizard=1`，WPF 当前直接套用“沥青路面-主线行车道”预设并打开原有参数窗口；编辑请求不写入该标记或写出 `0`，因此双击编辑不显示向导。
