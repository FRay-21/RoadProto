...
- 版本：`v0.1.23`
- 构建日期：`20260523`
- 阶段：`PavementLayerTemplateGeneralParams`
- ARX 文件名：`RoadProto_v0.1.23_20260523_PavementLayerTemplateGeneralParams.arx`
...
- 路面结构层模板命令：命令 `RD_SECTION_PAVEMENT_LAYER_TEMPLATE_CREATE`
  ...
  - 用户点取插入点后生成独立 `DnPavementLayerTemplateEntity`，并打开 WPF 参数窗口编辑模板名称、显示比例、预览宽度、显示方式、结构层列表、每层 RGB 颜色、填充类型、填充角度和填充比例；通用参数区默认只显示预览宽度、结构层数量、显示方式和当前编辑层，勾选“显示全部通用参数”后才显示结构代号、路基干湿类型、路面类型、路基土组、设计弯沉和累计轴次，这些新增字段先作为模板数据保存，不参与 DWG 标注或道路模型结构层显示。
  - 结构层类型下拉值固定为上面层、中面层、下面层、基层、底基层、垫层。
...
```text
ARXLOAD artifacts\x64\Release\RoadProto_v0.1.23_20260523_PavementLayerTemplateGeneralParams.arx
