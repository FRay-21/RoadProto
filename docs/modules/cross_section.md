...
- 模块编码：`CROSS_SECTION`
- 命令前缀：`RD_SECTION_`
- 当前状态：已实现路基模板独立实体创建、边坡模板独立实体创建、路面结构层模板独立实体创建、WPF 参数窗口、`.rpavement.xml` 导入导出、二维预览、双击编辑入口、桥接回写、路基部件点选绑定结构层模板和插入点夹点移动；已实现横断面戴帽道路模型创建、编辑、WPF 路基模板范围表、左右边坡模板组、模板组管理入口、生成进度反馈、`DnRoadModelEntity` 三维道路模型网格线框实体、路面结构层线框、断面地面快照、按采样桩号查看横断面预览和 ObjectARX 回写流程。
...
| domain | `src/domain/cross_section/PavementLayerTemplateModel.*` | 路面结构层模板枚举、默认值、等厚/非等厚、内外侧加宽/坡度规则和横断面预览几何构建 |
| domain | `src/domain/cross_section/RoadModel.*` | 道路模型配置、模板范围、路面结构层模板来源、边坡模板组、采样、TIN 地面剖切、断面节点链、结构层线框、三维网格线框和横断面预览领域模型 |
...
- 横断面道路模型生成时读取绑定的结构层模板，生成结构层三维线框并持久化到 `DnRoadModelEntity`。
