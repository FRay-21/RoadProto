...
- 验证状态：已通过托管 bridge 测试 Debug/Release 临时输出目录、核心测试 Debug/Release 构建与运行、WPF 托管插件 Release 构建和 `git diff --check`；默认 Debug/Release 托管测试输出目录仍被残留测试进程锁定。本轮未生成新的 ARX 版本文件。
