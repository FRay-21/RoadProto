...
public static class PavementLayerTemplateXmlFile
{
    ...
    public static PavementLayerTemplateDto Read(string path)
    {
        ...
        foreach (var element in root.Elements("Layer"))
        {
            ...
            var defaultColor = PavementLayerTemplateLabels.DefaultColorForLayerIndex(layerIndex);
            template.Layers.Add(new PavementLayerTemplateLayerDto
            {
                ...
                InnerWidening = ReadRequiredDouble(element, "innerWidening"),
                OuterWidening = ReadRequiredDouble(element, "outerWidening"),
    ...
    private static int ReadOptionalColorChannel(XElement element, string name, int fallback)
    {
        ...
        {
            throw new InvalidDataException($"Invalid pavement layer template XML color attribute: {name}.");
        }
