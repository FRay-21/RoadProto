...
- 版本：`v0.1.8`
- 构建日期：`20260509`
- 阶段：`AlignmentWpfPreview`
- ARX 文件名：`RoadProto_v0.1.8_20260509_AlignmentWpfPreview.arx`
...
在 AutoCAD 2021 中手动验证当前地形构网流程：

```text
ARXLOAD artifacts\x64\Release\RoadProto_v0.1.8_20260509_AlignmentWpfPreview.arx
NETLOAD artifacts\managed\Release\net48\RoadProto.Terrain.UI.dll
```

加载后可在 Ribbon 中打开 `RoadProto` 选项卡，点击 `数模` 面板下的 `地形构网`、`编辑数模`、`导出数模` 或 `导入数模`；也可点击 `平面设计` 面板下的 `平面布线`、`编辑平曲线参数`、`导出中线 ICD` 和 `导入中线 ICD`。命令行可直接运行 `DN_TERRAIN_TIN_CREATE`、`DN_TERRAIN_TIN_EDIT`、`DN_TERRAIN_TIN_EXPORT`、`DN_TERRAIN_TIN_IMPORT`、`RD_ALIGN_CENTERLINE_CREATE`、`RD_ALIGN_CURVE_PARAM_EDIT`、`RD_ALIGN_CENTERLINE_EXPORT_ICD` 和 `RD_ALIGN_CENTERLINE_IMPORT_ICD`。数模流转文件后缀固定为 `.rmesh`，道路中线流转文件后缀固定为 `.icd`。
