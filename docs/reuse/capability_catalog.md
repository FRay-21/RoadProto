...
| 路面结构层模板领域模型与默认值 | V0.1.19 原型，支持上面层/中面层/下面层/基层/底基层/垫层、等厚/内外侧非等厚、内外侧加宽和坡度、结构层横断面预览几何 | `src/domain/cross_section/PavementLayerTemplateModel.*` |
| 路面结构层模板自定义实体 | V0.1.19 原型，独立显示路面结构层模板，支持 DWG 持久化、几何范围、变换和双击编辑入口 | `src/cad_adapter/objectarx/cross_section/DnPavementLayerTemplateEntity.*` |
| 路面结构层模板 WPF 编辑 Bridge | V0.1.19 原型，通过请求/响应文件编辑路面结构层模板，WPF 支持 `.rpavement.xml` 导入导出 | `src/cad_adapter/objectarx/cross_section/PavementLayerTemplateDialogBridge.*` |
| 横断面道路模型 | V0.1.19 原型，按道路中线、竖曲线、路基模板范围、路面结构层模板引用和边坡模板组生成断面节点链、结构层线框、地面剖面快照与三维网格线框，支持优先级解析、行内点选模板、模板组管理、生成进度回调、TIN 网格候选剖切、采样桩号保存、横断面预览、DWG 持久化和编辑回写 | `src/domain/cross_section/RoadModel.*` |
...
- `PavementLayerTemplateRules` 统一处理结构层类型编码、显示名称、等厚/非等厚厚度、内外侧加宽、内外侧坡度和横断面预览几何。
- 内侧 = closer to road centerline for the subgrade component；外侧 = farther from road centerline。内侧加宽扩向道路中线。
- `DnPavementLayerTemplateEntity` 是 CAD 持久化和显示能力，负责保存模板数据、绘制预览、支持 DWG 保存重开和双击编辑。
- `PavementLayerTemplateDialogBridge` 是原型阶段 UI 解耦能力，通过请求/响应文件在 WPF 与 C++ ObjectARX 之间传递模板参数。
- `.rpavement.xml` 是 WPF 侧模板流转格式，可用于导入导出路面结构层模板参数，不替代 DWG 自定义实体持久化。
- 路基模板部件通过 handle 引用路面结构层模板；所有部件类型均允许绑定，是否生成结构层由 handle 是否有效决定。
- 道路模型生成读取绑定模板并创建结构层三维线框，`RoadModelSectionPreviewBuilder` 在查看横断面中显示 `结构层`。
