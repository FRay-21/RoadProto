...
| 横断面设计 | `CROSS_SECTION` | `RD_SECTION_` | 路基模板、边坡模板和路面结构层模板独立实体已实现；路面结构层模板支持 WPF 编辑、`.rpavement.xml` 导入导出和路基部件绑定；道路模型横断面戴帽已接入结构层三维边界线和弱化填充面、边坡模板组、生成进度、断面地面快照、查看横断面预览、双击编辑入口和 Bridge 回写 | `docs/modules/cross_section.md` |
