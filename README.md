...
- 版本：`v0.1.21`
- 构建日期：`20260523`
- 阶段：`PavementLayerTemplateAnnotation`
- ARX 文件名：`RoadProto_v0.1.21_20260523_PavementLayerTemplateAnnotation.arx`
...
- 路面结构层模板命令：命令 `RD_SECTION_PAVEMENT_LAYER_TEMPLATE_CREATE`
  ...
  - 用户点取插入点后生成独立 `DnPavementLayerTemplateEntity`，并打开 WPF 参数窗口编辑模板名称、显示比例、预览宽度、结构层列表和每层 RGB 颜色。
  ...
  - WPF 预览、DWG 模板实体和道路模型结构层使用每层保存的 RGB 颜色、预览式弱化填充和层色边线；模板实体额外显示层名/厚度/加宽/坡度标注。查看横断面预览也读取同一份层颜色。相邻层共线重叠的边界按同一几何线表达，避免非等厚结构层斜边重复或台阶化。
...
```text
ARXLOAD artifacts\x64\Release\RoadProto_v0.1.21_20260523_PavementLayerTemplateAnnotation.arx
