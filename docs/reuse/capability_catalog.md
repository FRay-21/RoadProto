...
| 路面结构层模板领域模型与默认值 | V0.1.22 原型，支持上面层/中面层/下面层/基层/底基层/垫层、每层 RGB 显示色、CAD 填充类型、填充角度、填充比例、显示方式、等厚/内外侧非等厚、内外侧正/负加宽和正/负坡度、顶边沿上一层底边所在直线延长或收回、坡度按 `1:n` 驱动顶边到底边的侧边水平移动、四边形/梯形横断面预览几何 | `src/domain/cross_section/PavementLayerTemplateModel.*` |
| 路面结构层模板自定义实体 | V0.1.22 原型，独立显示路面结构层模板，按层保存 RGB 真彩色、填充类型、填充角度和填充比例，支持按颜色/按填充/按填充+颜色显示，DWG 中仅在模板上方居中显示模板名称，不显示尺寸标注，支持 DWG 持久化、几何范围、变换和双击编辑入口 | `src/cad_adapter/objectarx/cross_section/DnPavementLayerTemplateEntity.*` |
| 路面结构层模板 WPF 编辑 Bridge | V0.1.22 原型，通过请求/响应文件编辑路面结构层模板、显示方式、每层 RGB、每层填充类型、填充角度和填充比例，WPF 支持当前层编辑、预览点击选层、固定字号标注、索引颜色选择和 `.rpavement.xml` 导入导出 | `src/cad_adapter/objectarx/cross_section/PavementLayerTemplateDialogBridge.*` |
...
- `PavementLayerTemplateRules` 统一处理结构层类型编码、显示名称、填充类型、填充角度、填充比例、显示方式、等厚/非等厚厚度、内外侧加宽、内外侧坡度和横断面预览几何。
...
- `.rpavement.xml` 是 WPF 侧模板流转格式，可用于导入导出路面结构层模板参数、显示方式、填充类型、填充角度和填充比例，不替代 DWG 自定义实体持久化。
