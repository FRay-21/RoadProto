...
- 统一传递 `showCreateWizard` 请求标记；该标记只用于新建流程触发路面结构层创建向导，按 handle 编辑既有实体时保持关闭。
...
- 结构层类型编码对应 WPF 下拉显示：上面层、中面层、下面层、沥青封层、基层、底基层、垫层、搭板层。
...
- 新建请求写出 `showCreateWizard=1`，WPF 先运行创建向导再打开原有参数窗口；编辑请求不写入该标记或写出 `0`，因此双击编辑不显示向导。
